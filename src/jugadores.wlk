...
//Compra-Venta al Banco
	method comprar(unaPropiedad){
		const confirmarCompra = new Popup(img="popups/confirmarCompra.png",position=game.at(1,2))
	...
	method transferencia() {
		const confirmarTransferenciaPopup = new Popup(img="popups/confirmarTransferencia.png",position=game.at(1,2))
...
//Alquileres
 	method pagarAlquiler(propiedad){
 		const alquilerPago = new Popup(img="popups/alquilerPago.png",position=game.at(1,2))
...
//Prestamos
	method pedirPrestamo(){
		...
		if (prestamosDisponibles > 0 and deuda <= 1400){
			const prestamoAlBanco = new Popup(img="popups/prestamoAlBanco" + prestamosDisponibles.toString() + ".png",position=game.at(1,2))
		...
		keyboard.n().onPressDo{ prestamoAlBanco.removeVisual() }
			...
			const historialCrediticio = new Popup(img="popups/historialCrediticio.png",position=game.at(1,2))
	...
	method pagarDeudas(){
		...
		if (dinero >= 600 and prestamosDisponibles < 3){
			...
			const pagarPrestamo = new Popup(img="popups/pagarPrestamo.png",position=game.at(1,2))
			...
			const deudasCanceladas = new Popup(img="popups/deudasCanceladas.png",position=game.at(1,2))
	...
	method pagarFianza(){
		if (dinero >= 1000){
			...
			const fianza = new Popup(img="popups/fianza.png",position=game.at(1,2))
			fianza.addVisual()
...
//Victoria, Derrota, y Evaluación de Condición.
	...
	method derrota(){
		...
		const derrotado = new Popup(img="popups/derrota" + frasesDeDerrota.anyOne().toString() + ".png",position=game.at(1,2),position=game.at(2,2))
